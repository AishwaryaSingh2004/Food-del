...
return ()=>{
  ...
  const verifyOtp = async () => {
    try {
      ...
      await axios.post(
        `${serverUrl}/api/order/verify-delivery-otp`, // ✅ FIXED API
        {
          orderId:currentOrder._id,
          shopOrderId:currentOrder.shopOrder._id,
          otp
          ...
          <div className='bg-white rounded-2xl shadow-md px-5 py-4 w-[90%]'>
            ...
            {!showOtpBox ? (
              ...
              <div className='mt-4 p-4 border rounded-xl bg-gray-50'>
                ...
                <button
                  className='mt-3 w-full bg-orange-500 text-white py-2 rounded-lg font-semibold hover:bg-orange-600 transition-all'
                  onClick={verifyOtp}
                >
                  Submit OTP
                </button>
...
function DeliveryBoy() {
  ...
  const verifyOtp = async () => {
    try {
      ...
      await axios.post(
        `${serverUrl}/api/order/verify-delivery-otp`, // ✅ FIXED
        {
          orderId:currentOrder._id,
          shopOrderId:currentOrder.shopOrder._id,
          otp
          ...
          <div className='bg-white rounded-2xl shadow-md px-5 py-4 w-[90%]'>
            ...
            {!showOtpBox ? (
              <button
                onClick={() => {
                  setShowOtpBox(true);
                  sendOtp();   // ✅ SEND OTP HERE
                }}
                className='mt-4 w-full bg-green-500 text-white font-semibold py-2 px-4 rounded-xl'
              >
                Mark As Delivered
              </button>
            ) : (
...
const getTodayDeliveries = async () => {
  try {
    ...
    console.log("TODAY DELIVERY ERROR:", error.response?.data || error.message);
  }
};
